...
if [[ "$CUSTOM_CURLRPM" = [yY] ]]; then
  ...
  ###############################################################
  # el7 64bit
  curl -${ipv_forceopt}Is --connect-timeout 30 --max-time 30 https://mirror.city-fan.org/ftp/contrib/yum-repo/city-fan.org-release-3-4.rhel7.noarch.rpm
  CURL_NOARCHRPMCHECK=$?
  if [[ "$CURL_NOARCHRPMCHECK" = '0' ]]; then
    rpm -Uvh https://mirror.city-fan.org/ftp/contrib/yum-repo/city-fan.org-release-3-4.rhel7.noarch.rpm
  else
    if [ -f "$DIR_TMP/city-fan.org-release-3-4.rhel7.noarch.rpm" ]; then
      rpm -Uvh "$DIR_TMP/city-fan.org-release-3-4.rhel7.noarch.rpm"
    fi
  fi
